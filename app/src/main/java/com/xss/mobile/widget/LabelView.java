...
public class LabelView extends View {
    ...
    public LabelView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mCenterPadding = ta.getDimension(R.styleable.LabelTextView_labelCenterPadding, dp2px(0));
        ...
        mTextColor = ta.getColor(R.styleable.LabelTextView_txtColor, Color.WHITE);
        mTextSize = ta.getDimension(R.styleable.LabelTextView_txtSize, sp2px(8));
        mText = ta.getString(R.styleable.LabelTextView_txt);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (Float.compare(mDegrees, (float)DEGREES_LEFT) == 0) {
            canvas.rotate(mDegrees, 0, height);
        } else if (Float.compare(mDegrees, (float)DEGREES_RIGHT) == 0) {
...
//        canvas.drawText(mNum, (width) / 2, (mTopPadding + mTextHeight + mCenterPadding + mNumHeight), mNumPaint);
        ...
        //绘制修饰文本
        canvas.drawText(mText, (float) ((width) * Math.sin(45) / 3), mTopPadding + mTextHeight + 2, mTextPaint);
    }

    @Override
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        super.onMeasure(widthMeasureSpec, heightMeasureSpec);
        height = (int) (mTopPadding + mCenterPadding + mBottomPadding + mTextHeight);
        width = 2 * height;
        //控件的真正高度，勾股定理...
